...
For a consequential interface shared across tasks, put the consumed and produced representation, authoritative owner, preconditions and deciding check in the existing plan/card. Compare tasks that actually share that boundary before editing; separate filenames do not prove independent ownership. A small table helps when several interfaces interact. Do not impose an all-pairs matrix or duplicate full implementation code in the plan.
