...
Establish the full task base/head range and relevant uncommitted changes; a task can span several commits, so do not assume `HEAD~1` is its complete diff. Include affected unchanged producers and consumers when the changed contract can reach them. An optional complexity finding must identify actual usage, an adequate existing capability and the maintenance consequence; an interface with one implementation or a larger diff is not itself a defect. Use the existing consolidated review rather than adding a simplicity reviewer.
