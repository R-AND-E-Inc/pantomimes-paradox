...
For a new regression check, name the plausible faulty behavior it should detect. Where practical, prove sensitivity against that defect in an isolated reversible fixture; distinguish the intended assertion failure from an import, setup or unavailable-environment failure. Preserve user work, and use characterization where existing behavior needs establishing. Do not delete working implementation to manufacture test-first history, set a fixed number of tests, or rerun a valid suite just because another status message is due.
