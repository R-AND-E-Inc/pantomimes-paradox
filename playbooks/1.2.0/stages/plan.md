...
Order independently reviewable slices toward the first useful checkpoint, proving untested boundaries early. Each slice states what action demonstrates its result. Plan the active unit from repository reality; reference the program instead of replanning it. Preserve contract amendment conventions and distinguish changed intent from a narrow amendment. If the existing contract can honestly close without a new idea, that idea is generally separate work.
