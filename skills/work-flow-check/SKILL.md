...
description: "Trace and exercise one complete user interaction from entry through persistence to the final visible result, when implementing or debugging it."
...
```sh
python3 "${CLAUDE_PLUGIN_ROOT}/scripts/resolve_workflow.py" --project-root "${CLAUDE_PROJECT_DIR}" --skill work-flow-check
```
...
```sh
python3 "<package root>/scripts/resolve_workflow.py" --project-root "$PWD" --skill work-flow-check
```
...
If `python3` is unavailable, read the adoption block in `docs/OPERATING.md`, take its `release`, and read `playbooks/<release>/core.md` and `playbooks/<release>/stages/flow-check.md` from the package root, stating that payload hashes were not verified.
