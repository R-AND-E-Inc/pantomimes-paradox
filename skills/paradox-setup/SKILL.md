...
In guided mode also list the twelve commands in one line each from `guide/commands.md`, and say that plain language works: the assistant picks the right skill from an ordinary request, so the commands are shortcuts, not homework.
