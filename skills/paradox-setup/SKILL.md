...
1. **Confirm the package works.** Run the resolver in bootstrap mode against the current directory. On Claude Code:

   ```sh
   python3 "${CLAUDE_PLUGIN_ROOT}/scripts/resolve_workflow.py" --project-root "${CLAUDE_PROJECT_DIR}" --skill work-start --bootstrap
   ```

   On Codex or any other assistant, the package root is two directories above this `SKILL.md`:

   ```sh
   python3 "<package root>/scripts/resolve_workflow.py" --project-root "$PWD" --skill work-start --bootstrap
   ```

   `status: ready` with `mode: bootstrap` means the package is intact and this folder has not adopted the workflow; report the `release`. `mode: adopted` (or the error `invalid_bootstrap`) means this project is already adopted: say so, run `work-resume` instead of continuing here, and stop. Any other error is reported verbatim with its `required_identity`; do not repair or install anything. If `python3` itself is unavailable, say so, read `${CLAUDE_PLUGIN_ROOT}/playbooks/manifest.json` to report the `bootstrap` release, note that payload hashes cannot be verified on this machine, and continue.

2. **Set the guidance mode.** If the user did not say, ask one question with a recommendation: *guided* (each reply explains what happened, why the next step matters, what they will see, and how to tell it worked), *expert* (ordinary replies, one-line handoffs), or *terse* (stripped to findings, decisions and what they must act on). Recommend guided for anyone who has not run a software project with an assistant before; offer terse only if they ask for minimum reading. Remember the answer for this session; it is written into the project's `docs/OPERATING.md` as `Guidance mode: <mode>` when `work-start` or `work-adopt` creates that file (every adopted project has one, even a small utility that otherwise has only a README).
